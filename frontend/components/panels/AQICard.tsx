...
function WxStat({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ textAlign: "center", background: T.chip, borderRadius: "9px", padding: ".55em .3em" }}>
      <div style={{ fontSize: ".68em", color: T.subInk }}>{label}</div>
      <div style={{ fontFamily: T.mono, fontWeight: 600, fontSize: "1.05em" }}>{value}</div>
    </div>
  );
}
...
export default function AQICard({
  ...
  if (!station) {
    return (
      ...
      >
        <div style={{ fontSize: "1.6em", marginBottom: ".2em", opacity: 0.5 }}>📍</div>
        <div style={{ fontSize: ".86em", fontWeight: 600, marginBottom: ".1em", color: T.ink }}>
          ยังไม่ได้เลือกสถานี
        </div>
  ...
  const temp = weatherLoading ? "…" : weather ? `${Math.round(weather.temp)}°` : "—";
  const humid = weatherLoading ? "…" : weather ? `${Math.round(weather.humidity)}%` : "—";

  return (
    <section
      aria-label="คุณภาพอากาศสถานีที่เลือก"
      className="cp-anim-rise"
      style={{ border: `1px solid ${T.line}`, borderRadius: "14px", overflow: "hidden" }}
    >
      {/* colored header */}
      <div style={{ background: cls.color, color: "#fff", padding: ".9em 1em" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: "1em" }}>{name}</div>
            {station.province && (
              <div style={{ fontSize: ".78em", opacity: 0.9 }}>จ.{station.province}</div>
            )}
          </div>
          <div
            style={{
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: ".4em", marginTop: ".5em" }}>
          <span style={{ fontFamily: T.mono, fontWeight: 600, fontSize: "3.4em", lineHeight: 0.9 }}>
            {station.pm25 ?? "—"}
          </span>
          <span style={{ fontSize: ".85em", opacity: 0.9, paddingBottom: ".5em" }}>
            µg/m³ · PM2.5
          </span>
        </div>
      </div>
      ...
      <div style={{ padding: ".85em 1em", background: T.panel }}>
        ...
        {showHistory && <HistoryChart points={historyPoints} loading={historyLoading} />}
      </div>
    </section>
  );
}
